...
class QARepository implements QARepositoryInterface
{
    ...
    public function getQuestionAnswers($productType, $userId, $productId){
        ...
        a.id as answerId,a.title as title, a.content as content, a.point as point,
    ...
    public function getQuestions2($productType){
        ...
        (CASE
        WHEN q.question = '' THEN (SELECT name FROM sections where id = q.section_id limit 1)
        ELSE q.question
     END) AS question,
        q.parent_id, q.type, q.note, q.product_type_id from questions q where product_type_id = ?", [$productType]);
        return $result;
    }
    public function getUserMark($productId){
        $result = DB::select("select user_mark_id from total_marks where  product_id =?",[$productId]);
        return $result;
    }
