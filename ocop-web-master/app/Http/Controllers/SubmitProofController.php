...
class SubmitProofController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if(Auth()->guard('web')->check()){
            ...
            $idUserTotalMark = $this->qaRepository->getUserMark($product->id);
            $riviewerList=array();
            $setOfQuestions=20;
            $whereCondition="";
            if(count($idUserTotalMark)>0){
                foreach ($idUserTotalMark as $value){
                    $riviewerList[]=$value->user_mark_id;
                }
            }
            if(in_array($user->id,$riviewerList)){
...
//			dd($proofs);
            ...
            return view('proof.submitproof', compact('proofs','questions','product','product2','entity', 'groupA','groupB','groupC', 'totalA', 'totalB', 'totalC'));
        }else{
